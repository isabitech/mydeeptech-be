...
const hvncCommandSchema = new mongoose.Schema(
    {
        command_id: {
            ...
            required: true,
            unique: true,
            default: () => `cmd_${Math.random().toString(36).substr(2, 9)}_${Date.now()}`,
            index: true
        },
        device_id: {
            type: String,
        ...
        parameters: {
            ...
            validate: {
                validator: function(v) {
                    ...
                    if (this.action === 'navigate' && (!v.url || typeof v.url !== 'string')) {
                        return false;
                    }
        ...
        status: {
            type: String,
            enum: ['pending', 'sent', 'acknowledged', 'executing', 'completed', 'failed', 'timeout', 'cancelled'],
            default: 'pending',
            index: true
        },
        sent_at: Date,
        ...
        metadata: {
            source: {
                type: String,
                enum: ['admin_dashboard', 'api', 'automation', 'scheduled', 'user_request'],
...
hvncCommandSchema.virtual('is_active').get(function() {
    return ['pending', 'sent', 'acknowledged', 'executing'].includes(this.status) && !this.is_expired;
});
...
hvncCommandSchema.methods.start = function() {
    if (!['acknowledged', 'pending'].includes(this.status)) {
        throw new Error('Command must be in acknowledged or pending status to start');
    }
...
hvncCommandSchema.statics.createCommand = function(commandData) {
    ...
    if (commandData.timeout_seconds) {
        expiresAt = new Date(Date.now() + (commandData.timeout_seconds + 60) * 1000); // Add 1 minute buffer
    }
...
hvncCommandSchema.statics.getCommandHistory = function(userEmail, deviceId, limit = 50) {
    const query = {};
...
hvncCommandSchema.statics.getBatchCommands = function(batchId) {
    return this.find({ 'metadata.batch_id': batchId })
        .sort({ createdAt: 1 });
};
...
hvncCommandSchema.index({ createdAt: 1 }, { 
