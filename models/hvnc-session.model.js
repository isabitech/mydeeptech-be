...
hvncSessionSchema.virtual("has_exceeded_max_duration").get(function () {
  const maxDurationMs =
    (this.settings.max_duration_hours || 8) * 60 * 60 * 1000;
...
hvncSessionSchema.methods.endSession = function (reason = "user_logout") {
  ...
  if (this.hubstaff_session?.is_timer_running) {
    ...
    this.hubstaff_session.total_minutes = Math.floor(
      (this.hubstaff_session.timer_ended_at -
        this.hubstaff_session.timer_started_at) /
        (1000 * 60),
    );
  }
    ...
    this.activity_summary.idle_periods.forEach((period) => {
      if (!period.end) {
        ...
        period.duration_minutes = Math.floor(
          (period.end - period.start) / (1000 * 60),
        );
      }
    });
  }
...
// Static methods
hvncSessionSchema.statics.createSession = async function (
  deviceId,
  userEmail,
  accessCodeId,
  ipAddress,
...
hvncSessionSchema.pre("save", function (next) {
  ...
  if (this.ended_at && !this.duration_minutes) {
    this.duration_minutes = Math.floor(
      (this.ended_at - this.started_at) / (1000 * 60),
    );
  }
