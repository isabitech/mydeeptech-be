 ...
 */
router.post('/admin/commands/send',
  authenticateAdmin,
  requirePermission('device_management'),
  async (req, res) => {
    try {
 ...
 */
router.get('/admin/devices/:deviceId',
  authenticateAdmin,
  requirePermission('device_management'),
 ...
 */
router.put('/admin/devices/:deviceId',
  authenticateAdmin,
  requirePermission('device_management'),
 ...
 */
router.post('/admin/devices/:deviceId/access-code/generate',
  authenticateAdmin,
  requirePermission('device_management'),
 ...
 */
router.post('/admin/devices/:deviceId/hubstaff/start',
  authenticateAdmin,
  requirePermission('device_management'),
 ...
 */
router.post('/admin/devices/:deviceId/hubstaff/pause',
  authenticateAdmin,
  requirePermission('device_management'),
 ...
 */
router.get('/admin/shifts/:shiftId',
  authenticateAdmin,
  requirePermission('shift_management'),
 ...
 */
router.put('/admin/shifts/:shiftId',
  authenticateAdmin,
  requirePermission('shift_management'),
 ...
 */
router.delete('/admin/shifts/:shiftId',
  authenticateAdmin,
  requirePermission('shift_management'),
 ...
 */
router.get('/admin/shifts/calendar',
  authenticateAdmin,
  requirePermission('shift_management'),
...
/**
 * Get all users for shift assignment (Admin only)
 * GET /api/hvnc/admin/shifts/users
 */
router.get('/admin/shifts/users',
  authenticateAdmin,
  requirePermission('shift_management'),
  async (req, res) => {
    try {
      ...
      // Get HVNC users
      let users = await HVNCUser.find({ status: 'active' })
        .select('_id full_name email role')
        .lean();
      
      // Also get MyDeepTech domain users from main collection
      const mydeeptechUsers = await DTUser.find({ 
        email: /@mydeeptech\.ng$/i,
        isEmailVerified: true 
      }).select('_id fullName email').lean();
      
      // Add MyDeepTech users to the list if not already present
      for (const user of mydeeptechUsers) {
...
/**
 * Get all devices for shift assignment (Admin only)
 * GET /api/hvnc/admin/shifts/devices
 */
router.get('/admin/shifts/devices',
  authenticateAdmin,
  requirePermission('shift_management'),
  async (req, res) => {
    try {
      ...
      const devices = await HVNCDevice.find({})
        .select('_id pc_name device_id status')
        .lean();
 ...
 */
router.post('/user/sessions/start',
  authenticateUserSession,
 ...
 */
router.post('/user/sessions/:sessionId/end',
  authenticateUserSession,
 ...
 */
router.post('/sessions/:sessionId/force-end',
  authenticateAdmin,
  requirePermission('device_management'),
