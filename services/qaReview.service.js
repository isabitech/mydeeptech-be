...
class QAReviewService {
    ...
    async getPendingSubmissions(query) {
        ...
        const submissions = await qaReviewRepository.findSubmissions({
            matchQuery,
            sort,
            skip,
        ...
        const totalCount = await qaReviewRepository.countSubmissions(matchQuery);
    ...
    async getSubmissionForReview(submissionId) {
        const submission = await qaReviewRepository.findSubmissionById(submissionId);
        ...
        const qaReview = await qaReviewRepository.findReviewBySubmissionId(submissionId);

        // Calculate metrics
        const totalScore = (submission.tasks || []).reduce((sum, task) => sum + (task.score || 0), 0);
        const averageScore = submission.tasks && submission.tasks.length > 0 ? totalScore / submission.tasks.length : 0;
        ...
        return {
            ...
            qaReview,
            isReviewed: !!qaReview && (qaReview.status === 'completed' || qaReview.reviewStatus === 'completed')
        };
    }
    ...
    async reviewTask(data, user) {
        ...
        if (qaReview.taskScores.length > 0) {
            const totalScoreSum = qaReview.taskScores.reduce((sum, task) => sum + task.totalScore, 0);
            qaReview.overallScore = Math.round(totalScoreSum / qaReview.taskScores.length);
        }
    ...
    async submitFinalReview(data, user) {
        ...
        if (!qaReview) {
            throw new Error('QA review not found. Please review individual tasks first.');
        }
    ...
    async getReviewerDashboard(user) {
        ...
        const stats = await qaReviewRepository.getReviewerStats(reviewerId);

        // Get recent reviews
        const recentReviewsRaw = await qaReviewRepository.getRecentReviews(reviewerId);

        // Get pending review count
        const pendingCount = await qaReviewRepository.countSubmissions({
            status: 'submitted',
            submittedAt: { $exists: true, $ne: null }
        });
    ...
    async getQAStatsByReviewer(reviewerId, query) {
        ...
        const stats = await qaReviewRepository.getQAStatsByReviewer(reviewerId, startDate, endDate);
        ...
        return {
            ...
            revisionRate: Number(((s.revisionRequestedCount / total) * 100).toFixed(1)),
