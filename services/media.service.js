  ...
  class MediaService {
      formatFileData(file, options = {}) {
          ...
          if (options.isImage || file.mimetype?.startsWith('image/')) {
              ...
              fileData.optimizedUrl = generateOptimizedUrl(file.filename, options.optimizeImage || { width: 800, height: 600 });
          }
  
          if (options.isVideo || file.mimetype?.startsWith('video/')) {
          ...
          if (options.isAudio || file.mimetype?.startsWith('audio/')) {
              fileData.duration = file.duration || null;
              fileData.streamingUrl = generateOptimizedUrl(file.filename, { 
                  resource_type: 'video',
                  format: 'mp3',
      ...
      async uploadMultiple(files, user) {
          if (!files || files.length === 0) throw { status: 400, message: 'No image files provided' };
  
          const filesData = files.map(file => this.formatFileData(file, { isImage: true }));
      ...
      async deleteFile(publicId, user) {
          if (!publicId) throw { status: 400, message: 'Public ID is required for file deletion' };
      ...
      async updateProfilePicture(file, userId) {
          ...
          const profilePictureData = this.formatFileData(file, { isImage: true, optimizeImage: { width: 300, height: 300 } });
