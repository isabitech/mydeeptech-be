...
const { sendPaymentConfirmation, sendPaymentReminder } = require('../utils/paymentMailer');
...
class InvoiceService {
    async createInvoice(value, adminId) {
        ...
        if (dtUser.annotatorStatus !== 'approved') {
            throw { status: 400, message: "Can only create invoices for approved annotators" };
        }
        ...
        if (!userWorkedOnProject) {
            throw { status: 400, message: "User has not worked on this project or application not approved" };
        }
    ...
    async getAllInvoices(query) {
        ...
        const invoices = await InvoiceRepository.fetchAll(filter, skip, limit);
        const totalInvoices = await InvoiceRepository.count(filter);
        const summary = await InvoiceRepository.getSummaryPattern(filter);
    ...
    async updatePaymentStatus(invoiceId, body) {
        const { paymentStatus, paymentMethod, paymentReference, paymentNotes, paidAmount } = body;
    ...
    async sendInvoiceReminder(invoiceId) {
        ...
        try {
            ...
            throw { status: 500, message: "Failed to send payment reminder", error: error.message };
        }
    }
    ...
    async deleteInvoice(invoiceId) {
        ...
        if (invoice.paymentStatus !== 'unpaid' || hoursAgo > 24) {
            throw { status: 400, message: "Can only delete unpaid invoices created within the last 24 hours" };
        }
    ...
    async bulkAuthorizePayment(adminEmail) {
        const unpaidInvoices = await InvoiceRepository.find({ paymentStatus: { $in: ['unpaid', 'overdue'] }});
    ...
    async buildInvoiceFilter(invoiceIds) {
        ...
        if (invoiceIds && Array.isArray(invoiceIds) && invoiceIds.length > 0) {
            ...
            if (invalidIds.length > 0) {
                throw { status: 400, message: "Invalid invoice ID(s) provided", details: { invalidIds, totalProvided: invoiceIds.length, validIds: validObjectIds.length } };
            }
            if (validObjectIds.length === 0) {
                throw { status: 400, message: "No valid invoice IDs provided" };
            }
            invoiceFilter = { _id: { $in: validObjectIds }, paymentStatus: { $in: ['unpaid', 'overdue'] } };
            ...
            invoiceFilter = { _id: new mongoose.Types.ObjectId(invoiceIds), paymentStatus: { $in: ['unpaid', 'overdue'] } };
        }
    ...
    async generatePaystackCSV(invoiceIds) {
        ...
        if (unpaidInvoices.length === 0) {
            return { 
                csvContent: '', 
                summary: { totalInvoices: 0, processedInvoices: 0, totalAmountUSD: 0, totalAmountNGN: 0, errors: [], selectedInvoices: invoiceIds ? invoiceIds.length : 0 }
            };
        }
        ...
        try {
            ...
            throw { status: 503, message: "Cannot generate CSV due to exchange rate service failure", error: "Exchange rate service unavailable", details: { exchangeRateError: rateError.message, totalInvoices: unpaidInvoices.length }};
        }

        const csvRows = [];
        const results = { totalInvoices: unpaidInvoices.length, selectedInvoices: invoiceIds ? invoiceIds.length : 0, processedInvoices: 0, totalAmountUSD: 0, totalAmountNGN: 0, errors: [] };
        csvRows.push(['Transfer Amount', 'Transfer Note (Optional)', 'Transfer Reference (Optional)', 'Recipient Code (This overrides all other details if available)', 'Bank Code or Slug', 'Account Number', 'Account Name (Optional)', 'Email Address (Optional)']);
        ...
        for (const invoice of unpaidInvoices) {
            try {
                ...
                if (!validation.isValid) {
                    results.errors.push({ userId: user._id, userEmail: user.email, invoiceNumber: invoice.invoiceNumber, error: 'Invalid payment info', details: validation.errors.join(', ') });
                    continue;
                }
                ...
                if (!bankCode) {
                    results.errors.push({ userId: user._id, userEmail: user.email, invoiceNumber: invoice.invoiceNumber, error: 'Bank code not found', details: `Unable to map bank: ${user.payment_info.bank_name}` });
                    continue;
                }
                ...
                results.errors.push({ invoiceNumber: invoice.invoiceNumber, error: 'Processing failed', details: invoiceError.message });
            }
        }
    ...
    async generateMPESACSV(invoiceIds) {
        ...
        if (unpaidInvoices.length === 0) {
            return { 
                csvContent: '', 
                summary: { totalInvoices: 0, processedInvoices: 0, totalAmountUSD: 0, errors: [], selectedInvoices: invoiceIds ? invoiceIds.length : 0 }
            };
        }

        const csvRows = [];
        const results = { totalInvoices: unpaidInvoices.length, selectedInvoices: invoiceIds ? invoiceIds.length : 0, processedInvoices: 0, totalAmountUSD: 0, errors: [] };
        csvRows.push(['Transfer Amount(USD)', 'Transfer Note (Optional)', 'Transfer Reference (Optional)', 'MPESA Account Number', 'Account Name', 'Email Address']);

        for (const invoice of unpaidInvoices) {
            try {
                const user = invoice.dtUserId;
                if (!invoice.invoiceAmount || invoice.invoiceAmount <= 0) {
                    results.errors.push({ userId: user._id, userEmail: user.email, invoiceNumber: invoice.invoiceNumber, error: 'Invalid invoice amount', details: `Amount is ${invoice.invoiceAmount || 'undefined'}` });
                    continue;
                }
                if (!user.payment_info?.account_number) {
                    results.errors.push({ userId: user._id, userEmail: user.email, invoiceNumber: invoice.invoiceNumber, error: 'Missing MPESA account number', details: 'payment_info.account_number is required' });
                    continue;
                }
                if (!user.payment_info?.account_name) {
                    results.errors.push({ userId: user._id, userEmail: user.email, invoiceNumber: invoice.invoiceNumber, error: 'Missing account name', details: 'payment_info.account_name is required' });
                    continue;
                }
                ...
                results.errors.push({ userId: invoice.dtUserId?._id, userEmail: invoice.dtUserId?.email, invoiceNumber: invoice.invoiceNumber, error: 'Processing failed', details: invoiceError.message });
            }
        }
