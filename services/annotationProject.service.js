...
class AnnotationProjectService {
  ...
  getAllProjects = async (query) => {
    ...
    const projects = await this.repository.findAllProjects(filter, skip, limit);
    const totalProjects = await this.repository.countProjects(filter);

    // Summary stats
    const activeProjects = await this.repository.countProjects({ ...filter, isActive: true });
    const completedProjects = await this.repository.countProjects({ ...filter, status: 'completed' });
    const pausedProjects = await this.repository.countProjects({ ...filter, status: 'paused' });

    const statusSummary = await this.repository.aggregateProjects([
      { $group: { _id: '$status', count: { $sum: 1 } } }
    ]);

    const categorySummary = await this.repository.aggregateProjects([
  ...
  getProjectDetails = async (projectId, search) => {
    const project = await this.repository.findProjectByIdWithPopulate(projectId, [
    ...
    const applicationStats = await this.repository.aggregateApplications([
    ...
    const recentApplications = await this.repository.aggregateApplications(recentApplicationsPipeline);
    const approvedAnnotators = await this.repository.aggregateApplications(buildSearchPipeline('approved', 'reviewedAt'));
    const rejectedAnnotators = await this.repository.aggregateApplications(buildSearchPipeline('rejected', 'reviewedAt'));
    const pendingAnnotators = await this.repository.aggregateApplications(buildSearchPipeline('pending', 'appliedAt'));
    ...
    const recentReviewActivity = await this.repository.aggregateApplications([
      { $match: { projectId: project._id, status: { $in: ['approved', 'rejected'] }, reviewedAt: { $exists: true } } },
      {
        $lookup: {
          from: 'dtusers',
          localField: 'applicantId',
      ...
      { $sort: { reviewedAt: -1 } },
      { $limit: 10 },
      { $project: { status: 1, reviewedAt: 1, reviewedBy: { fullName: 1, email: 1 }, applicantId: { fullName: 1, email: 1 }, reviewNotes: 1, rejectionReason: 1 } }
    ]);
  ...
  updateProject = async (projectId, updateData) => {
    const project = await this.repository.updateProject(projectId, { ...updateData, updatedAt: new Date() });
  ...
  requestDeletionOTP = async (projectId, admin, reason) => {
    ...
    const allApplicationsCount = await this.repository.countApplications({ projectId });
  ...
  verifyOTPAndDelete = async (projectId, otp, admin, confirmationMessage) => {
    ...
    const allApplications = await this.repository.findApplications({ projectId });
  ...
  getApplications = async (query) => {
    ...
    const totalResult = await this.repository.aggregateApplications(countPipeline);
    const totalApplications = totalResult.length > 0 ? totalResult[0].total : 0;

    pipeline.push({ $skip: skip }, { $limit: limit });
    const applications = await this.repository.aggregateApplications(pipeline);
    ...
    const statusSummary = await this.repository.aggregateApplications(summaryPipeline);
  ...
  approveApplication = async (applicationId, admin, reviewNotes) => {
    const application = await this.repository.findApplicationByIdWithPopulate(applicationId, [
      { path: 'projectId', select: 'projectName projectCategory payRate approvedAnnotators maxAnnotators projectGuidelineLink projectGuidelineVideo projectCommunityLink projectTrackerLink' },
      { path: 'applicantId', select: 'fullName email' }
    ]);

    if (!application) throw new Error('Application not found');
    if (application.status !== 'pending') throw new Error(`Application is already ${application.status}`);
    ...
    await this.repository.updateProject(project._id, { $inc: { approvedAnnotators: 1 } });
    ...
    try {
      ...
      await MailService.sendProjectApprovalNotification(application.applicantId.email, application.applicantId.fullName, projectData);
    } catch (emailError) {
  ...
  rejectApplication = async (applicationId, admin, rejectionReason, reviewNotes) => {
    const application = await this.repository.findApplicationByIdWithPopulate(applicationId, [
    ...
    try {
      ...
      await MailService.sendProjectRejectionNotification(application.applicantId.email, application.applicantId.fullName, projectData);
    } catch (emailError) {
    ...
    try {
      await NotificationService.createApplicationStatusNotification(
        ...
        'rejected',
        { _id: application.projectId._id, projectName: application.projectId.projectName, projectCategory: application.projectId.projectCategory },
        { _id: application._id }
      );
  ...
  removeApprovedApplicant = async (applicationId, admin, removalReason, removalNotes) => {
    const application = await this.repository.findApplicationByIdWithPopulate(applicationId, [
      { path: 'projectId', select: 'projectName projectCategory approvedAnnotators' },
      { path: 'applicantId', select: 'fullName email phone' }
    ]);

    if (!application) throw new Error('Application not found');
    if (application.status !== 'approved') throw new Error(`Only approved applicants can be removed (Current: ${application.status})`);
    ...
    await this.repository.updateProject(application.projectId._id, { $inc: { approvedAnnotators: -1 } });

    try {
      await MailService.sendApplicantRemovalNotification(originalData.applicantEmail, originalData.applicantName, {
        ...
        workPeriod: { startedAt: originalData.workStartedAt, endedAt: application.workEndedAt }
      });

      await MailService.sendProjectAnnotatorRemovedNotification(admin.email, admin.fullName || 'Administrator', {
        projectName: originalData.projectName,
        projectId: originalData.projectId,
        removedApplicant: { name: originalData.applicantName, email: originalData.applicantEmail },
        removalReason: application.removalReason,
        removedAt: application.removedAt,
    ...
    return {
      ...
      applicant: { name: originalData.applicantName, email: originalData.applicantEmail },
      project: { id: originalData.projectId, name: originalData.projectName },
      removal: { removedAt: application.removedAt, removedBy: admin.email, reason: application.removalReason },
      workPeriod: { startedAt: originalData.workStartedAt, endedAt: application.workEndedAt, duration: application.workEndedAt - originalData.workStartedAt },
  ...
  getRemovableApplicants = async (projectId) => {
    ...
    const approvedApplications = await this.repository.findApplications({ projectId, status: 'approved' }, { reviewedAt: -1 });
    ...
    return {
      project: { id: project._id, name: project.projectName, totalApprovedAnnotators: project.approvedAnnotators || 0, maxAnnotators: project.maxAnnotators },
      removableApplicants,
      summary: { totalRemovableApplicants: removableApplicants.length, canRemoveAll: true, projectStatus: project.status }
    };
  };
  ...
  exportAnnotatorsCSV = async (projectId) => {
    ...
    const approvedApplications = await this.repository.findApplications({ projectId, status: 'approved' }, { reviewedAt: -1 });
    if (approvedApplications.length === 0) throw new Error('No approved annotators found');
    ...
    approvedApplications.forEach(app => {
      ...
      const row = [`"${applicant.fullName || 'N/A'}"`, `"${personalInfo.country || 'N/A'}"`, `"${applicant.email || 'N/A'}"`];
      csvRows.push(row.join(','));
    });

    const timestamp = new Date().toISOString().split('T')[0];
    const filename = `${project.projectName.replace(/[^a-zA-Z0-9\s]/g, '').replace(/\s+/g, '_').toLowerCase()}_approved_annotators_${timestamp}.csv`;
  ...
  attachAssessment = async (projectId, body, admin) => {
    ...
    if (!assessmentConfig) throw new Error('Assessment configuration not found');

    const adminId = admin.userId || admin.userDoc?._id;

    project.assessment = { isRequired, assessmentId, assessmentInstructions, attachedAt: new Date(), attachedBy: adminId };
    await project.save();
    return await this.repository.findProjectByIdWithPopulate(project._id, [{ path: 'assessment.assessmentId', select: 'title description numberOfTasks estimatedDuration' }]);
  };
  ...
  removeAssessment = async (projectId) => {
    ...
    await project.save();
    return { id: project._id, name: project.projectName, hadAssessment: wasRequired };
  };

  getAvailableAssessments = async () => {
    const assessments = await MultimediaAssessmentConfig.find({ isActive: true }).populate('projectId', 'projectName').sort({ createdAt: -1 }).lean();
    return assessments.map(assessment => ({
      ...
      approvalRate: assessment.statistics?.totalSubmissions > 0 ? (assessment.statistics.approvedSubmissions / assessment.statistics.totalSubmissions * 100).toFixed(1) : 0
    }));
  };

  getApprovedApplicants = async (projectId) => {
    if (!this.repository.isValidObjectId(projectId)) throw new Error('Invalid project ID');
    const project = await this.repository.findProjectById(projectId);
    if (!project) throw new Error('Project not found');

    return await this.repository.findApplications({ projectId, status: 'approved' }, { reviewedAt: -1 });
  };

  rejectApplicationsBulk = async (body) => {
    const { applicationIds, admin, rejectionReason = 'other', reviewNotes = '' } = body;
    if (!admin || !admin.userId) throw new Error('Admin information is required');
    if (!Array.isArray(applicationIds) || applicationIds.length === 0) throw new Error('No application IDs provided');

    const validIds = applicationIds.filter(id => this.repository.isValidObjectId(id));
    if (!validIds.length) throw new Error('No valid application IDs provided');

    const applications = await this.repository.findApplications({ _id: { $in: validIds }, status: 'pending' });
    ...
    await this.repository.updateApplicationsMany(
      ...
      { $set: { status: 'rejected', reviewedBy: admin.userId, reviewedAt: new Date(), rejectionReason, reviewNotes } }
    );

    const notificationResults = await Promise.allSettled(
      applications.map(async application => {
        try {
          await MailService.sendProjectRejectionNotification(application.applicantId.email, application.applicantId.fullName, {
            projectName: application.projectId.projectName,
            projectCategory: application.projectId.projectCategory,
            adminName: admin.fullName,
            rejectionReason,
          ...
          await NotificationService.createApplicationStatusNotification(application.applicantId._id, 'rejected', application.projectId, application);
    ...
    const successCount = notificationResults.filter(r => r.status === 'fulfilled' && r.value.status === 'success').length;
    return {
  ...
  bulkApproveApplications = async (body, admin) => {
    ...
    if (!Array.isArray(applicationIds) || applicationIds.length === 0) throw new Error('applicationIds must be a non-empty array');
    ...
    for (const applicationId of applicationIds) {
      try {
        ...
        if (application.status !== 'pending') {
          failed.push({ applicationId, reason: `Application already ${application.status}` });
          continue;
        }
        ...
        const updatedProject = await this.repository.updateProject(
          { _id: project._id, ...(project.maxAnnotators && { approvedAnnotators: { $lt: project.maxAnnotators } }) },
          { $inc: { approvedAnnotators: 1 } },
          { new: true }
        );

        if (!updatedProject) {
          failed.push({ applicationId, reason: 'Project has reached maximum number of annotators' });
          continue;
        }
        ...
        try {
          await MailService.sendProjectApprovalNotification(application.applicantId.email, application.applicantId.fullName, {
            projectName: project.projectName, projectCategory: project.projectCategory, payRate: project.payRate, adminName: admin.fullName,
            reviewNotes: reviewNotes || '', projectGuidelineLink: project.projectGuidelineLink, projectGuidelineVideo: project.projectGuidelineVideo,
            projectCommunityLink: project.projectCommunityLink, projectTrackerLink: project.projectTrackerLink
          });
        } catch (e) { }

        approved.push({ applicationId, applicantName: application.applicantId.fullName, projectName: project.projectName });
      } catch (e) {
        failed.push({ applicationId, reason: e.message });
      }
    }

    return { totalRequested: applicationIds.length, approvedCount: approved.length, failedCount: failed.length, approved, failed };
  };
  ...
  bulkRejectApplications = async (body, admin) => {
    ...
    for (const applicationId of applicationIds) {
      try {
        ...
        try {
          await MailService.sendProjectRejectionNotification(application.applicantId.email, application.applicantId.fullName, {
            projectName: application.projectId.projectName, projectCategory: application.projectId.projectCategory, adminName: admin.fullName,
            rejectionReason: application.rejectionReason, reviewNotes: reviewNotes || ''
          });
          await NotificationService.createApplicationStatusNotification(application.applicantId._id, 'rejected', { _id: application.projectId._id, projectName: application.projectId.projectName, projectCategory: application.projectId.projectCategory }, { _id: application._id });
        } catch (e) { }

        rejected.push({ applicationId, applicantName: application.applicantId.fullName, projectName: application.projectId.projectName });
      } catch (e) {
        failed.push({ applicationId, reason: e.message });
      }
    }

    return { totalRequested: applicationIds.length, rejectedCount: rejected.length, failedCount: failed.length, rejected, failed };
  };
  ...
  getAvailableProjects = async (userId, query, user) => {
    ...
    const limit = parseInt(query.limit) || 10;
    const skip = (page - 1) * limit;
    const { view = "available", category, difficultyLevel, applicationStatus, search, payRateMin, payRateMax } = query;
    ...
    const allUserApplications = await this.repository.findApplications({ applicantId: userId });
    const applicationMap = new Map(allUserApplications.map(app => [app.projectId.toString(), {
      applicationId: app._id,
      status: app.status,
      appliedAt: app.appliedAt,
      reviewedAt: app.reviewedAt,
    ...
    const appliedProjectIds = allUserApplications.map(app => app.projectId.toString());

    let finalFilter = { ...filter };
    if (view === "available") {
      if (appliedProjectIds.length > 0) {
        finalFilter._id = { $nin: appliedProjectIds.map(id => this.repository.toObjectId(id)) };
      }
    } else if (view === "applied") {
      if (appliedProjectIds.length === 0) {
        return {
          projects: [],
          pagination: { currentPage: page, totalPages: 0, totalProjects: 0, hasNext: false, hasPrev: false, limit },
          filters: { view, category, difficultyLevel, applicationStatus },
          userInfo: this._formatUserInfo(user, allUserApplications)
        };
      }
      finalFilter._id = { $in: appliedProjectIds.map(id => this.repository.toObjectId(id)) };
      if (applicationStatus) {
        ...
        finalFilter._id = { $in: filteredAppIds.map(id => this.repository.toObjectId(id)) };
      }
    }
    ...
    for (const project of projects) {
      const appCount = await this.repository.countApplications({
      ...
      if (project.applicationDeadline) {
        ...
        project.applicationOpen = new Date() < deadline;
        project.daysUntilDeadline = Math.ceil((deadline - new Date()) / (1000 * 60 * 60 * 24));
    ...
    return {
      ...
      filters: { view, applicationStatus, category, difficultyLevel, search, payRateMin, payRateMax },
      userInfo: this._formatUserInfo(user, allUserApplications)
    };
  };
  ...
  applyToProject = async (userId, projectId, body) => {
    ...
    const { coverLetter, proposedRate, availability, estimatedCompletionTime } = body || {};
    ...
    if (requiresAssessment) {
      if (user.multimediaAssessmentStatus === "failed") {
        ...
        if (Date.now() < cooldownEnd.getTime()) {
          ...
          err.hoursRemaining = Math.ceil((cooldownEnd - new Date()) / (60 * 60 * 1000));
          throw err;
        }
      }

      if (user.multimediaAssessmentStatus !== "approved") {
        const assessmentConfig = await MultimediaAssessmentConfig.findById(project.assessment.assessmentId).populate("projectId", "projectName");
        ...
        await MailService.sendAssessmentInvitationEmail(user.email, user.fullName, {
    ...
    await this.repository.updateProject(projectId, { $inc: { totalApplications: 1 } });

    // Send project application notification to admins
    if (!assessmentTriggered) {
      try {
        const projectWithAdmins = await this.repository.findProjectByIdWithPopulate(projectId, [
          { path: "createdBy", select: "fullName email" },
          { path: "assignedAdmins", select: "fullName email" }
        ]);
        ...
        for (const email of adminEmails) {
          await MailService.sendProjectApplicationNotification(email, "Project Admin", notificationData);
        }
      } catch (err) {
        console.error("⚠️ Failed to notify admins of application:", err.message);
      }
    }

    return await this.repository.findApplicationByIdWithPopulate(application._id, [
  ...
  manuallyAddUserToProject = async (projectId, userId, adminId) => {
    ...
    if (existingApplication) {
      if (existingApplication.status === 'approved') throw new Error("already_approved");
