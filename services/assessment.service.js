...
const submitAssessmentSchema = Joi.object({
  assessmentType: Joi.string()
    .valid("annotator_qualification", "skill_assessment", "project_specific")
...
const submitAssessment = async ({ userId, body, ipAddress, userAgent }) => {
  try {
    if (!userId) throw buildError(401, "User authentication required");

    const { error, value } = submitAssessmentSchema.validate(body);
    if (error)
      throw buildError(400, "Validation error", mapValidationErrors(error));
    ...
    const canRetake = await Assessment.canUserRetake(
      userId,
      assessmentType,
      24,
    );
    if (!canRetake)
      throw buildError(
        400,
        "You must wait 24 hours before retaking the assessment",
      );
    ...
    if (statusChanged) {
      ...
      try {
        ...
        console.error(
          "⚠️ Failed to create status change notification:",
          notificationError,
        );
      }
      ...
      try {
        if (passed && newAnnotatorStatus === "approved") {
          ...
          } = require("../utils/annotatorMailer");
          await MailService.sendAnnotatorApprovalEmail(
            user.email,
            user.fullName,
          );
        ...
        attemptInfo: {
          ...
          previousBestScore:
            previousAttempts.length > 0
              ? Math.max(...previousAttempts.map((a) => a.scorePercentage))
              : null,
        },
      },
    };
    ...
    throw buildError(500, "Failed to submit assessment", error.message);
  }
};

const getUserAssessmentHistory = async ({ userId, query }) => {
  try {
    if (!userId) throw buildError(401, "User authentication required");

    const { error, value } = getAssessmentsSchema.validate(query);
    if (error)
      throw buildError(400, "Validation error", mapValidationErrors(error));
    ...
    throw buildError(500, "Failed to fetch assessment history", error.message);
  }
};
...
const checkRetakeEligibility = async ({
  ...
  try {
    ...
    const latestAttempt = await Assessment.getUserLatestAttempt(
      userId,
      assessmentType,
    );
    ...
    if (!canRetake && latestAttempt) {
      nextRetakeTime = new Date(
        latestAttempt.createdAt.getTime() + 24 * 60 * 60 * 1000,
      );
    }
    ...
    throw buildError(500, "Failed to check retake eligibility", error.message);
  }
};

const getAdminAssessments = async ({ query, isAdmin }) => {
  try {
    if (!isAdmin) throw buildError(401, "Admin authentication required");
    ...
    const overallStats = await Assessment.aggregate([
      ...
      },
      {
        $addFields: {
          ...
          passRate: {
            $multiply: [
              { $divide: ["$passedAssessments", "$totalAssessments"] },
              100,
            ],
          },
        },
    ...
    throw buildError(500, "Failed to fetch assessments", error.message);
  }
};
...
const getAssessmentQuestions = async ({
  ...
  try {
    ...
    if (language === "akan") {
      ...
      const parsed = parseInt(questionsPerSection, 10);
      actualQuestionsPerSection = Number.isNaN(parsed) ? 5 : parsed;
    }

    // ✅ Fisher-Yates shuffle (correct randomness)
    const shuffle = (arr) => {
    ...
    const queries = sections.map((section) => {
      let matchQuery = { section, isActive: true };
      ...
      return AssessmentQuestion.aggregate([
        { $match: matchQuery },
    ...
    // ✅ Run all section queries in parallel (faster)
    const results = await Promise.all(queries);

    // ✅ Flatten + randomize options safely
    const allQuestions = results.flat().map((q) => ({
      ...q,
    ...
    const timeLimit = language === "akan" ? 35 : 30;
...
const getSectionStatistics = async ({ isAdmin, requireAdminCheck = false }) => {
  try {
    if (requireAdminCheck && !isAdmin) {
      throw buildError(401, "Admin authentication required");
    }
    ...
    const performanceStats = await Assessment.aggregate([
      ...
      },
      {
        $addFields: {
          successRate: {
            $multiply: [
              { $divide: ["$correctAnswers", "$totalAnswered"] },
              100,
            ],
          },
        },
    ...
    throw buildError(500, "Failed to fetch section statistics", error.message);
  }
};
...
const getAllAssessments = async ({ userId }) => {
  try {
    ...
    // Toggle optional assessment types; keep false to mirror legacy (English only)
    const includeAkanAssessments = true;
    const includeMultimediaAssessments = true;
    const includeSpideyAssessments = true;
    ...
    const englishAssessment = {
      ...
      description:
        "Comprehensive assessment covering grammar, vocabulary, comprehension, and writing skills",
      category: "language",
      difficulty: "intermediate",
      ...
      instructions:
        "This assessment evaluates your English language proficiency across four key areas. You have 30 minutes to complete 20 questions with a minimum score of 60% required to pass.",
      benefits: [
    ...
    if (latestEnglishAttempt) {
      const cooldownEnd = new Date(
        latestEnglishAttempt.createdAt.getTime() + 24 * 60 * 60 * 1000,
      );
    ...
    if (includeAkanAssessments) {
      const akanAssessment = {
        id: "akan-proficiency",
        type: "akan_proficiency",
        title: "Akan (Twi) Proficiency Assessment",
        description:
          "Comprehensive assessment covering Akan grammar, vocabulary, translation, writing, and reading skills",
        category: "language",
        difficulty: "intermediate",
        ...
        instructions:
          "This assessment evaluates your Akan (Twi) language proficiency across five key areas: Grammar, Vocabulary, Translation, Writing, and Reading. You have 35 minutes to complete 50 questions with a minimum score of 60% required to pass.",
        benefits: [
      ...
      if (latestAkanAttempt) {
        const cooldownEnd = new Date(
          latestAkanAttempt.createdAt.getTime() + 24 * 60 * 60 * 1000,
        );
        const canRetakeNow = new Date() >= cooldownEnd;
    ...
    if (includeMultimediaAssessments) {
      ...
      try {
        const multimediaAssessments = await MultimediaAssessmentConfig.find({
          ...
          .populate(
            "projectId",
            "projectName projectCategory projectDescription",
          )
          .select(
            "title description requirements scoring videoReels projectId createdAt",
          );
        ...
        for (const config of multimediaAssessments) {
          const multimediaAssessment = {
            ...
            cooldownHours:
              config.requirements?.retakePolicy?.cooldownHours || 24,
            isActive: true,
            projectInfo: {
            ...
            instructions:
              config.instructions ||
              "Complete multimedia annotation tasks to demonstrate your skills in video analysis and content categorization.",
            benefits: [
            ...
            videoReels: {
              ...
              categories: Object.keys(
                config.videoReels?.reelsPerNiche || {},
              ).filter((key) => config.videoReels.reelsPerNiche[key] > 0),
            },
          };

          const user = await DTUser.findById(userId).select(
            "multimediaAssessmentStatus multimediaAssessmentAttempts multimediaAssessmentLastAttempt multimediaAssessmentLastFailedAt",
          );
          ...
          if (user?.multimediaAssessmentLastFailedAt) {
            const cooldownEnd = new Date(
              user.multimediaAssessmentLastFailedAt.getTime() +
              multimediaAssessment.cooldownHours * 60 * 60 * 1000,
            );
            multimediaAssessment.userStatus.canRetake =
        ...
        console.error(
          "⚠️ Error fetching multimedia assessments:",
          multimediaError,
        );
      }
    }

    if (includeSpideyAssessments) {
      try {
        ...
        for (const config of spideyConfigs) {
          const spideyAssessment = {
            ...
            instructions:
              "This is a quality enforcement engine designed to protect partner accounts. Any rule violation results in immediate failure. The assessment uses a 4-stage state machine with hard rules and integrity testing.",
            benefits: [
            ...
            stages: [
              ...
              },
              {
                ...
                description:
                  "Automated validation with forbidden keyword detection",
              },
              {
              ...
              },
              {
                ...
                description:
                  "Blind compliance detection and violation flagging",
              },
            ],
            warnings: [
          ...
          if (
            ...
            spideyAssessment.userStatus.nextRetakeAvailable =
              "No retakes allowed for Spidey assessment";
          }
      ...
      body: {
        ...
        data: {
          ...
          summary: {
            ...
            englishProficiency: assessments.filter(
              (a) => a.type === "english_proficiency",
            ).length,
            multimediaAssessments: assessments.filter(
              (a) => a.type === "multimedia_assessment",
            ).length,
            spideyAssessments: assessments.filter(
              (a) => a.type === "spidey_assessment",
            ).length,
            userCanTake: assessments.filter((a) => a.userStatus.canRetake)
              .length,
          },
          instructions: {
            english:
            ...
            multimedia:
              "Complete multimedia assessments to access specialized video annotation projects",
            spidey:
              "⚠️ Spidey is a high-discipline assessment with zero tolerance for violations - ONE ATTEMPT ONLY",
            general:
              "Each assessment has specific requirements and cooldown periods between attempts",
          },
        },
      },
    };
    ...
    throw buildError(
      500,
      "Failed to fetch available assessments",
      error.message,
    );
  }
};

const startAssessmentById = async ({ userId, assessmentId, query }) => {
  try {
    if (!userId) throw buildError(401, "User authentication required");
    ...
    if (assessmentId === "english-proficiency") {
      ...
      if (latestAttempt) {
        ...
        if (new Date() < cooldownEnd) {
          throw buildError(
            400,
            "Assessment cooldown active. Please wait before retaking.",
            {
      ...
      const questionsResponse = await getAssessmentQuestions({
        userId,
        language: "en",
        questionsPerSection: query?.questionsPerSection,
      });
    ...
    if (!assessmentConfig || !assessmentConfig.isActive) {
      throw buildError(404, "Assessment not found or not available");
    }
    ...
    if (user.multimediaAssessmentLastFailedAt) {
      const cooldownEnd = new Date(
        user.multimediaAssessmentLastFailedAt.getTime() +
        assessmentConfig.requirements.retakePolicy.cooldownHours *
        60 *
        60 *
        1000,
      );
    ...
    throw buildError(500, "Failed to start assessment", error.message);
  }
};

const getAssessmentSubmissions = async ({
  assessmentId,
  query,
  requestingUserId,
  isAdmin,
  ...
  try {
    ...
    if (!isAdmin && userId && userId !== requestingUserId) {
      throw buildError(
        403,
        "Access denied. You can only view your own submissions unless you are an admin.",
      );
    }
    ...
    if (assessmentId === "english-proficiency") {
      ...
      const assessments = await Assessment.find(filter)
        .populate(
          "userId",
          "fullName email annotatorStatus microTaskerStatus qaStatus",
        )
        .select("-questions.correctAnswer -ipAddress -userAgent")
      ...
      assessmentInfo = {
        ...
        title: "English Proficiency Assessment",
        description:
          "Annotator qualification assessment covering comprehension, vocabulary, grammar, and writing",
        passingScore: 60,
        totalQuestions: 20,
        sections: ["Comprehension", "Vocabulary", "Grammar", "Writing"],
      };
      ...
      const assessmentConfig =
        await MultimediaAssessmentConfig.findById(assessmentId);
      if (!assessmentConfig)
        throw buildError(404, "Multimedia assessment not found");
      ...
      const multimediaSubmissions = await MultimediaAssessmentSubmission.find(
        filter,
      )
        .populate(
          "annotatorId",
      ...
      submissions = multimediaSubmissions.map((submission) => ({
        ...
        submission: {
          ...
          tasksCompleted:
            submission.tasks?.filter((task) => task.isCompleted).length || 0,
    ...
    const statistics = submissions.reduce(
      (acc, sub) => {
        if (sub.type === "english_proficiency") {
          ...
          acc.averageScore =
            ((acc.averageScore || 0) * (acc.processedCount || 0) +
              sub.submission.scorePercentage) /
            ((acc.processedCount || 0) + 1);
        } else {
      ...
      { total: totalCount, currentPage: submissions.length },
    );
      ...
      body: {
        ...
        data: {
          ...
          pagination: {
            ...
            totalCount,
            hasNext: parseInt(page, 10) * parseInt(limit, 10) < totalCount,
...
const getAdminAssessmentsOverview = async ({ isAdmin }) => {
  try {
    if (!isAdmin) throw buildError(401, "Admin authentication required");
    ...
    try {
      ...
      const completionRate =
        englishData.totalSubmissions > 0
          ? (englishData.approvedSubmissions / englishData.totalSubmissions) *
          100
          : 0;
      ...
      assessments.push({
        ...
        title: "English Proficiency Assessment",
        description:
          "Evaluate English language skills including grammar, vocabulary, and comprehension.",
        type: "english_proficiency",
        ...
        averageCompletionTime:
          englishData.averageCompletionTime * 1000 || 1800000,
        createdAt:
          englishData.firstSubmissionAt || new Date("2023-12-01T08:00:00Z"),
        isActive: true,
    ...
    const overallStats = assessments.reduce(
      (acc, assessment) => {
    ...
    overallStats.averageCompletionRate =
      overallStats.activeAssessments > 0
        ? Math.round(
          (overallStats.averageCompletionRate /
            overallStats.activeAssessments) *
          10,
        ) / 10
        : 0;
...
const getUserAssessmentsOverview = async ({ userId }) => {
  try {
    if (!userId) throw buildError(401, "User authentication required");
    ...
    try {
      ...
      const bestAttempt = userEnglishAttempts.reduce(
        (best, current) =>
          !best || current.scorePercentage > best.scorePercentage
            ? current
            : best,
        null,
      );
      ...
      assessments.push({
        ...
        title: "English Proficiency Assessment",
        description:
          "Evaluate your English language skills including grammar, vocabulary, and comprehension.",
        type: "english_proficiency",
        numberOfTasks: 20,
        estimatedDuration: 30,
        timeLimit: 30,
      ...
      console.error(
        "Error fetching user English assessment data:",
        englishError,
      );
      assessments.push({
        id: "english_proficiency",
    ...
    assessments.sort((a, b) => {
      ...
      if (a.userStatus !== b.userStatus) {
        ...
        return (
          (statusPriority[a.userStatus] || 4) -
          (statusPriority[b.userStatus] || 4)
        );
      }

      const aDate = new Date(
        a.lastAttempt?.completedAt || a._internal?.createdAt || Date.now(),
      );
      const bDate = new Date(
        b.lastAttempt?.completedAt || b._internal?.createdAt || Date.now(),
      );
    ...
    const userStats = {
      ...
      inProgressAssessments: assessments.filter(
        (a) => a.userStatus === "in_progress",
      ).length,
      notStartedAssessments: assessments.filter(
        (a) => a.userStatus === "not_started",
      ).length,
      qualificationLevel: {
        hasEnglishQualification:
          assessments.find((a) => a.id === "english_proficiency")
            ?.userStatus === "completed" || false,
        multimediaQualifications: assessments.filter(
          (a) => a.type === "multimedia" && a.userStatus === "completed",
        ).length,
        overallLevel: (() => {
          const englishCompleted =
            assessments.find((a) => a.id === "english_proficiency")
              ?.userStatus === "completed";
          const multimediaCompleted = assessments.filter(
            (a) => a.type === "multimedia" && a.userStatus === "completed",
          ).length;
            ...
            suggestedAssessment:
              assessments.find(
                (a) =>
                  ...
                  (!a._internal?.requirements?.prerequisites ||
                    a._internal.requirements.prerequisites.length === 0),
              )?.id || null,
          },
        },
      },
