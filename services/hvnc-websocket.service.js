...
const initializeHVNCSocket = (server) => {
  ...
  try {
    ...
    if (!io) {
      console.error(
        "❌ Socket.IO instance not found. Chat service must be initialized first.",
      );
      return;
    }
  ...
  deviceNamespace.on("connection", async (socket) => {
    ...
    socket.on("session_event", async (data) => {
      try {
        ...
        await HVNCActivityLog.logSessionEvent(
          session_id,
          "session_event",
          {
            event,
    ...
    socket.on("command_result", async (data) => {
      try {
        ...
        await HVNCActivityLog.logCommandEvent(
          command_id,
          "command_executed",
          {
            status,
  ...
  adminNamespace.on("connection", (socket) => {
    ...
    socket.emit(
      "device_states",
      Array.from(connectedDevices.entries()).map(([deviceId, conn]) => ({
        device_id: deviceId,
  ...
  userNamespace.on("connection", (socket) => {
    ...
    socket.on("start_session", async (data) => {
      try {
        ...
        console.log(
          `🎮 Session started: ${user.fullName} → ${deviceConnection.device.pc_name}`,
        );
      } catch (error) {
    ...
    socket.on("disconnect", (reason) => {
      console.log(
        `👤 User disconnected from HVNC: ${user.fullName} (${reason})`,
      );
      ...
      for (const [sessionId, sessionData] of activeSessions.entries()) {
        if (sessionData.user._id.toString() === user._id.toString()) {
          ...
          HVNCSession.findById(sessionId)
            .then((session) => {
              if (session) {
                session.ended_at = new Date();
            ...
            .catch((err) =>
              console.error("Error ending session on disconnect:", err),
            );
