...
const buildDeviceReference = (device) => ({
  id: device._id,
  deviceId: device.deviceId,
  name: device.name,
});

const buildDeviceOperationLog = ({
  level,
  action,
  message,
  user,
  ...
  source: "api",
  ...(user ? { user } : {}),
  ...(device ? { device: buildDeviceReference(device) } : {}),
  ...(resource ? { resource } : {}),
  ...(metadata ? { metadata } : {}),
});

const buildDeviceErrorLog = ({ action, message, user, error }) => ({
  level: "error",
  category: "device",
  action,
  message,
...
class DeviceService {
  ...
  static async getDeviceLogs(deviceId, queryParams = {}, requestContext = {}) {
    try {
      ...
      const logs = await LogRepository.getDeviceLogs(
        deviceId,
        startDate,
        endDate,
        limit,
  ...
  static async assignDeviceUser(
    ...
    try {
      const device = await DeviceRepository.assignUser(
        deviceId,
        userId,
        permissions,
      );
