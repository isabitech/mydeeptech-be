  ...
  try {
    ...
    try {
      ...
      if (validationError.errors && validationError.errors.session_id) {
        ...
        console.log('❌ FAIL: Unexpected validation error', validationError.message);
        allTestsPassed = false;
      }
    }
    ...
    const websocketServicePath = path.join(__dirname, '../services/hvnc-websocket.service.js');
    ...
    if (foundIssues.length === 0) {
      ...
      console.log(`❌ FAIL: Found ${foundIssues.length} problematic device references:`);
      foundIssues.forEach(issue => console.log(`   ${issue.substring(0, 80)}...`));
      allTestsPassed = false;
    }

    // Test 3: Verify socket.device is used consistently
    console.log('\n📋 Test 3: Consistent socket.device usage...');
    const socketDeviceUsages = websocketCode.match(/socket\.device\.(device_id|pc_name|status|save|updateHeartbeat)/g)?.length || 0;
    if (socketDeviceUsages > 0) {
      console.log(`✅ PASS: Found ${socketDeviceUsages} proper socket.device usages`);
    } else {
    ...
    const enumUsages = websocketCode.match(/logDeviceEvent\([^,]+,\s*["']([^"']+)["']/g);
    const invalidEnums = enumUsages?.filter(usage => 
      usage.includes('"device_connected"') || usage.includes("'device_connected'")
    );
    ...
    const sessionCreationMatch = websocketCode.match(/HVNCSession\.create\(\{[^}]*session_id:\s*session_id/s);
    ...
    for (const pattern of authBypassPatterns) {
      const matches = websocketCode.match(pattern);
      if (matches) {
        console.log(`❌ FAIL: Found debug/test authentication bypass: ${matches[0]}`);
