...
class SessionManager {
  ...
  async handleStartSession(ws, data) {
    ...
    try {
      ...
      ws.send(JSON.stringify({ type: 'session_start_failed', error: error.message }));
    }
  }
  ...
  async handleEndSession(ws, data) {
    try {
      ...
      ws.send(JSON.stringify({ type: 'session_end_failed', error: error.message }));
    }
  }
...
const getSessionStats = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch session statistics' });
  }
};
